...
resolution = [1366, 768] if debug == 1 else [1366, 768]
...
win = visual.Window(resolution, units='deg', monitor=mon, fullscr = False)
...
image_folder = r'C:\Users\u0103239\Desktop\ruimte_experiment\Ruimte'
instruction_images = {}
for i in range(1,27): ##Hier heb ik nu 1 - 29 van gemaakt, dus dat zijn alle afbeeldingen, ook diegene tussen de blokken door. 
...
endPracticeText = visual.TextStim(win,'Dit waren alle oefenbeurten. Nu begint onze echte reis! Wanneer je juist antwoordt, zal je score omhoog gaan. Enkel wanneer je fout antwoordt, krijg je feedback.  Druk op de spatiebalk als je er klaar voor bent.',color='white')
###############################################################################
...
for trial in trials:
    ...
    if  trial.totBlockN%18==0 and trial.trialN==0:
        if trial.totBlockN==0:
            ...
            if 0 <= opponentScore < (totalScore-3):
                opponentScore = np.random.randint(opponentScore, totalScore-3)
            
            playerScoreText = visual.TextStim(win,str(int(totalScore)),height=26,pos=(239-640,-(645-360)), units='pix',color='black')
            opponentScoreText = visual.TextStim(win,str(opponentScore),height=26,pos=(429-640,-(645-360)),units='pix',color='black')
...
playerScoreText = visual.TextStim(win,str(int(totalScore)),height=26,pos=(239-640,-(645-360)), units='pix',color='black')
opponentScoreText = visual.TextStim(win,str(opponentScore),height=26,pos=(429-640,-(645-360)),units='pix',color='black')
instruction_images[str(25)].draw()
playerScoreText.draw()
opponentScoreText.draw()
win.flip()
event.waitKeys(keyList=['space'])
